...
def evaluate(model, test_loader, model_path, save_dir, device):
    ...
    with open(os.path.join(save_dir, "report.txt"), "w") as f:
        f.write("=== EEG Conformer Denoising Evaluation Report ===\n")
    ...
    # Plot sample reconstructions (up to 5 batches)
    ...
    for i in range(min(5, len(test_loader))):
        ...
        # Plot first feature dimension aggregated over time (or first channel)
        # If input is (B, T, F), we plot the first feature across T.
        plt.subplot(5, 1, i + 1)
        plt.plot(clean[0, :, 0].detach().cpu().numpy(), label='Clean', alpha=0.7)
        plt.plot(denoised[0, :, 0].detach().cpu().numpy(), label='Denoised', alpha=0.7)
        plt.plot(noisy[0, :, 0].detach().cpu().numpy(), label='Noisy', alpha=0.4)
        if i == 0:
            plt.legend()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate EEG Conformer denoiser")
    ...
    parser.add_argument('--split', type=str, default='testing_epochs', help='Split to evaluate (testing_epochs/validation_epochs)')
    parser.add_argument('--out', type=str, default=EVAL_PATH, help='Output dir for evaluation report')
    args = parser.parse_args()

    dataset = EEGDataset(args.data, split=args.split)
    setattr(dataset, 'transform', None)
    test_loader = TorchDataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=0)

    # Infer num_features from a batch
    try:
    ...
    save_dir = os.path.join(args.out, f'eeg_conformer_eval_{timestamp}')

    evaluate(model, test_loader, args.model, save_dir, DEVICE)
